  ...
  <div class="container" :style="{maxWidth: `${letterCount * 82 + 50}px`}">
    ...
    <div class="mt-10 text-center" v-if="options.length > 0">
      ...
      <div class="solve-options mt-5">
        <v-sheet color="" border rounded class="solve-option" v-for="option in showOptions" @click="addWordRow(option)">
          {{ option }}
        </v-sheet>
      </div>
...
function checkWordForCounts(word, countObject, min = true) {
  let wordCounts = {}
